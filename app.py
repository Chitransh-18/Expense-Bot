...
def get_expenses(current_user):
    ...
    filter_type = request.args.get("filter", "all") # 'all', 'this_month', 'last_month'
    ...
    if filter_type == "this_month":
        rows = [
            r for r in rows if r["date"].startswith(now.strftime("%Y-%m"))
        ]
...
def get_recurring_bills(current_user):
    ...
    # Process due status for each bill
    bills_processed = []
    for bill in rows:
...
def add_recurring_bill(current_user):
    data = request.get_json() or {}
    title = data.get("title", "").strip() # e.g. "YouTube Family Premium"
    total_amount = float(data.get("total_amount", 0))
    user_share = float(data.get("user_share", 0))
    paid_by = data.get("paid_by", "Self").strip() # e.g. "Daksh"
    due_day = int(data.get("due_day", 1)) # 1 to 31
...
def settle_recurring_bill(current_user, bill_id):
    """Mark recurring bill as settled for the current month and log expense"""
    conn = get_db()
    cursor = conn.cursor()
